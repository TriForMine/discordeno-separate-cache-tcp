...
export const cache = {
    ...
    members: new Collection<bigint, (GuildMemberWithUser & { guildId: string })[]>(),
                        ...
                        try {
                            if (requestEvent.request.url.endsWith('/memory')) {
                                return requestEvent.respondWith(
                                    new Response(JSON.stringify({success: true, response: Deno.memoryUsage()}), {
                                        headers: {
                            ...
                            if (requestEvent.request.headers.get('Content-Type') === 'application/msgpack') {
                            ...
                            const responseData = encode({success: true, response}, {extensionCodec});
